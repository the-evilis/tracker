...
| `npm run test:e2e` | Smoke в настоящем Chromium: демо-режим, отметки, виды, модалка, тема, PWA, все пять разделов, сроки и архив целей, категории «100», неделя принципов, бюджеты, повторы и импорт CSV (34 теста в двух профилях) |
...
|---|---|---|
| **Focus** | Цели с задачами, сроком, привязанными привычками и архивом; список «100» отдельной карточкой — с категориями, датами выполнения и заметками | `user_settings`, ключи `goals` и `list100` |
| **Money** | Доходы, расходы, бюджеты по категориям, повторяющиеся операции, импорт CSV, сравнение с прошлым месяцем, графики | таблица `transactions`; правила и лимиты — `user_settings`, ключи `money_rules` и `budgets` |
| **Tracker** | Привычки по дням: виды «Сегодня», неделя, месяц, год | `habits` + `user_settings` |
| **Credo** | Принципы, принцип дня, неделя по каждому принципу, связка с привычкой, заметка за день | `user_settings`, ключ `credo`; отметки — в `habits` |
| **Quotes** | Цитата дня (можно забрать в свою подборку) и свои цитаты с избранным | `user_settings`, ключ `quotes` |

Подробности, которые легко нарушить:

- **Вид «Сегодня»** показывает только сегодняшние плановые привычки: невыполненные сверху, сделанные ниже и приглушённые, внеплановые — отдельной группой.
- **Сроки целей** считаются в днях от сегодня, подпись человеческая («осталось 3 дня», «просрочено на 2 дня»).
- **Привычки цели** хранятся списком id в `habitIds`; карточка показывает средний процент за месяц по ним.
- **Бюджет** — лимит на категорию расходов в копейках; при 80% полоса желтеет, при 100% краснеет.
- **Повторяющиеся операции** собираются из уже введённых операций месяца и добавляются кнопкой; у добавленной операции остаётся `ruleId`, чтобы не задвоить её в том же месяце.
- **Импорт CSV** ищет колонки по названиям («дата», «сумма», «описание»), понимает `;` и `,` как разделитель, форматы дат `ГГГГ-ММ-ДД` и `ДД.ММ.ГГГГ`; знак суммы определяет доход это или расход. Неразобранные строки не теряются молча — их число показывается в итоге.
